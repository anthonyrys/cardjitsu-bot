...
async def clear_lobby(lb: lobby) -> None:
    ...
    for usrs in lb.users: active_user_pool.remove(usrs)
...
async def display_options(lb: lobby) -> None:
    ...
    # creating message for each user in lobby
...
async def on_option_pick(
    ...
    if (lobby_players[0].selected_card != None and lobby_players[1].selected_card != None):
